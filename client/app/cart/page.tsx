...
// Menu items data - same as in menu page
const menuItemsData = [
  // Pizza Items
  {
    id: 1,
...
const CartPage: React.FC = () => {
  ...
  // Get cart items with full details - FIXED VERSION
  const getCartItems = (): CartItem[] => {
    const cartItems: CartItem[] = [];
    
    Object.entries(cart).forEach(([itemId, quantity]) => {
      const item = menuItemsData.find(menuItem => menuItem.id === parseInt(itemId));
      if (item) {
        cartItems.push({
          id: item.id,
          name: item.name,
  ...
  const updateCartQuantity = (itemId: number, quantity: number) => {
    ...
    setCart(newCart);
    localStorage.setItem('foodExpressCart', JSON.stringify(newCart));
  };
  ...
  const clearCart = () => {
    setCart({});
    localStorage.setItem('foodExpressCart', JSON.stringify({}));
  };
  ...
  const handleContinueShopping = () => {
    // Get the last selected category from localStorage or use 'all'
    const lastCategory = localStorage.getItem('foodExpressLastCategory') || 'all';
    
    // Navigate to menu page with the same category
    router.push('/menu');
  };
        ...
        <div className="flex flex-col lg:flex-row gap-8">
          ...
          <div className="lg:w-2/3">
                ...
                {cartItems.map((item) => (
                  ...
                  >
                    ...
                    <div className="relative w-24 h-24 flex-shrink-0">
                      ...
                      />
                      {item.isPopular && (
                        <div className="absolute top-1 left-1 bg-orange-500 text-white px-2 py-1 rounded-full text-xs font-semibold">
                          Popular
                        </div>
                      )}
                    </div>
            ...
            <div className="bg-white rounded-2xl shadow-lg p-6">
              <button
                onClick={handleContinueShopping}
                className="flex items-center justify-center space-x-2 text-orange-600 hover:text-orange-700 font-semibold text-lg w-full"
              >
                <span>←</span>
